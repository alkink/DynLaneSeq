...
def batched_pairwise_range_aware_row_strip_iou(
    ...
    *,
    ...
    """Batched equivalent of :func:`pairwise_range_aware_row_strip_iou`.

    GT tensors may be padded along their lane axis.  A padded lane has no
    valid rows and therefore receives zero quality exactly like an invalid
    lane in the scalar implementation.
    """

    if pred_x_rows.ndim != 3 or gt_x_rows.ndim != 3:
        raise ValueError(
            "batched pred_x_rows and gt_x_rows must have shape [B,lanes,rows]"
        )
    batch, num_predictions, rows = pred_x_rows.shape
    if pred_range_norm.shape != (batch, num_predictions, 2):
        raise ValueError(
            "batched pred_range_norm must have shape [B,num_predictions,2]"
        )
    if gt_valid_mask.shape != gt_x_rows.shape:
        raise ValueError("batched gt_valid_mask must match gt_x_rows")
    if int(gt_x_rows.shape[0]) != int(batch) or int(gt_x_rows.shape[-1]) != int(rows):
        raise ValueError("batched prediction and GT row dimensions must match")
    ...
    num_gt = int(gt_x.shape[1])
    if int(num_predictions) == 0 or num_gt == 0:
        return (
            pred_x.new_zeros((batch, num_predictions, num_gt)),
            torch.zeros(
                (batch, num_predictions),
                dtype=torch.bool,
    ...
    pred_valid = (
        (y_rows.view(1, 1, -1) >= pred_range[..., :1] * float(input_h))
        & (y_rows.view(1, 1, -1) <= pred_range[..., 1:] * float(input_h))
