...
class S0Criterion(nn.Module):
    ...
    def compute_four_slot_selection_loss(
        ...
        training_real_route_logits = outputs.get(
            "selection_slot_training_real_route_logits"
        )
        if isinstance(training_real_route_logits, torch.Tensor):
            real_route_logits_for_loss = training_real_route_logits
        else:
            real_route_logits_for_loss = real_route_logits
        if isinstance(active_logits, torch.Tensor) and isinstance(
            real_route_logits_for_loss,
            torch.Tensor,
        ):
