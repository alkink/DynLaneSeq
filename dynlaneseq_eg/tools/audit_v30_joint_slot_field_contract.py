...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Require the direct route-residual gradient to remain disabled. "
            "The dense field loss must still reach the shared representation."
        ),
    )
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Require route selection loss to reach the proposal/image row "
            "representation while direct coordinate readout heads and the "
            "joint-field route residual remain protected."
        ),
    )
...
def main() -> None:
    ...
    route_scale = float(selector.joint_slot_field.route_residual_scale)
    checks = {
    ...
    if args.field_only:
        checks["route_residual_scale_exact_zero"] = route_scale == 0.0
        checks["route_loss_blocked_from_field"] = (
            route_norms["joint_field"] == 0.0
        )
    else:
        checks["route_loss_can_open_field_gate"] = (
            route_norms["joint_field"] > 0.0
        )
    if args.selection_gradient_bridge:
        bridge_scale = float(
            selector.selection_row_token_gradient_scale
        )
        checks.update(
            {
                "selection_row_bridge_enabled": bridge_scale > 0.0,
                "route_loss_reaches_backbone": route_norms["backbone"] > 0.0,
                "route_loss_reaches_fpn": route_norms["fpn"] > 0.0,
                "route_loss_reaches_row_projection": route_norms[
                    "row_feature_projection"
                ]
                > 0.0,
                "route_loss_does_not_directly_reach_coordinate_readouts": (
                    route_norms["coordinate_readout_heads"] == 0.0
                ),
                "route_loss_still_blocked_from_joint_field": (
                    route_norms["joint_field"] == 0.0
                ),
            }
        )
    ...
    report = {
        "experiment": (
            "V30 field-only zero-step contract"
            if args.field_only
            else "V30 joint slot-field zero-step contract"
        ),
        ...
        "route_residual_scale": route_scale,
        "selection_row_token_gradient_scale": bridge_scale,
