...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Summarize the V6-C router/refiner co-training gate."
    )
    ...
    parser.add_argument("--min-f1-050-gain", type=float, default=0.005)
    parser.add_argument("--max-f1-075-drop", type=float, default=0.005)
    parser.add_argument("--max-boundary-mass", type=float, default=0.45)
    parser.add_argument("--max-semantic-duplicate-fraction", type=float, default=0.01)
...
def _method(payload: dict) -> dict:
    ...
    if not isinstance(method, dict):
        raise ValueError("V6-C report has no four_slot_refined method")
    return method
...
def main() -> None:
    args = parse_args()
    if not args.report:
        raise ValueError("at least one V6-C trajectory report is required")
        ...
        rows.append(
            {
                ...
                "route_entropy": float(diagnostics.get("mean_route_entropy", 0.0)),
                "route_repair_fraction": float(
                    diagnostics.get("global_assignment_repair_fraction", 0.0)
                ),
                "semantic_duplicate_fraction": float(
                    diagnostics.get("semantic_duplicate_cluster_fraction", 1.0)
                ),
    ...
    eligible = [
        row
        for row in rows
        if row["f1_075"] >= source_075 - float(args.max_f1_075_drop)
    ]
    best = max(eligible or rows, key=lambda row: (row["f1_050"], row["f1_075"]))
    checks = {
        "primary_f1_materially_improved": best["f1_050"]
        >= source_050 + float(args.min_f1_050_gain),
        "strict_f1_preserved": best["f1_075"]
        >= source_075 - float(args.max_f1_075_drop),
        ...
        "semantic_duplicates_controlled": best["semantic_duplicate_fraction"]
        <= float(args.max_semantic_duplicate_fraction),
        "deployment_count_controlled": 3.0 <= best["mean_selected"] <= 3.5,
    }
    ...
    payload = {
        ...
        "gate": {
            "min_f1_050_gain": float(args.min_f1_050_gain),
            "max_f1_075_drop": float(args.max_f1_075_drop),
            "max_boundary_probability_mass": float(args.max_boundary_mass),
            "max_semantic_duplicate_fraction": float(
                args.max_semantic_duplicate_fraction
            ),
        ...
        "passed": passed,
        "next_step": (
            "full_validation_then_iterative_refinement_gate"
            if passed
            else "stop_and_reinspect_route_temperature_and_gradient_alignment"
        ),
    }
