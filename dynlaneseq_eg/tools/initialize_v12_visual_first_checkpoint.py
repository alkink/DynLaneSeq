...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Create deterministic V12 visual-first Stage-A initialization "
...
def main() -> None:
    ...
    if selector.unified_slot_decoder is not None:
        raise ValueError("V12 must not instantiate the V11 decoder")
    if selector.visual_first_association is None:
        raise ValueError("V12 visual-first association is unavailable")
    ...
    if not new_state:
        raise ValueError("V12 initialization found no new parameter tensors")
    ...
    report = {
        "experiment": "V12 deterministic visual-first Stage-A initialization",
        ...
        "v12_visual_first_association_present": True,
