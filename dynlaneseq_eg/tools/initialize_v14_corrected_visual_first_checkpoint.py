...
MODULE_PREFIX = (
    "structured_query_head.set_selection_head.corrected_visual_first_association"
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Create deterministic V14 corrected Stage-A initialization "
            "on top of the exact V7 deployment model."
        )
    )
...
def main() -> None:
    ...
    if selector.unified_slot_decoder is not None:
        raise ValueError("V14 must not instantiate the V11 decoder")
    if selector.corrected_visual_first_association is None:
        raise ValueError("V14 corrected visual-first association is unavailable")
    ...
    if not new_state:
        raise ValueError("V14 initialization found no new parameter tensors")
    ...
    report = {
        "experiment": "V14 deterministic corrected Stage-A initialization",
        "source_checkpoint": str(source_path),
        "source_iteration": source_iteration,
        "seed": int(args.seed),
        "compatible_load": stats,
        "v7_refiner_present": True,
        "v11_unified_slot_decoder_present": False,
        "v14_corrected_visual_first_association_present": True,
