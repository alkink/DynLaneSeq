...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Fast official-CULane validation sweep from cached model candidates. "
            "The model is forwarded once per checkpoint; score/quality thresholds "
            "are evaluated from cached candidates and official raster IoU matrices."
        )
    )
    ...
    parser.add_argument("--score-thresholds", type=float, nargs="+", default=[0.30, 0.35, 0.40, 0.45, 0.50, 0.55, 0.60])
    parser.add_argument("--quality-powers", type=float, nargs="+", default=[0.25, 0.50, 0.75])
    parser.add_argument("--iou-thresholds", type=float, nargs="+", default=[0.50, 0.55, 0.60, 0.65, 0.70, 0.75, 0.80, 0.85, 0.90, 0.95])
...
def _resolve_stage_names(cache: Dict[str, Any], requested: str) -> List[str]:
    stage_names = sorted({name for record in cache.get("records", []) for name in record.get("stages", {})})
    if not stage_names:
        raise RuntimeError("No prediction stages found in candidate cache.")
    if requested in stage_names:
        return [requested]
    if requested:
        raise KeyError(f"Requested stage={requested!r} not found. Available stages: {stage_names}")
    return [stage_names[-1]]


def _counts_for_selection(official_iou: torch.Tensor, selected_ids: Sequence[int], threshold: float) -> Tuple[int, int, int]:
...
def _row_sort_key(row: Dict[str, Any], selection_iou: float, tie_break_iou: float) -> Tuple[float, float, float, float]:
...
def main() -> None:
    ...
    stage_names = _resolve_stage_names(cache, args.stage)
    ...
    for stage_name in stage_names:
        for quality_power in quality_powers:
            for score_threshold in score_thresholds:
                counts: Dict[float, List[int]] = {threshold: [0, 0, 0] for threshold in iou_thresholds}
                image_count = 0
                selected_count = 0
                for record in tqdm(
                    cache["records"],
                    ...
                    if official_iou is None:
                        raise RuntimeError("Official IoU cache missing after ensure_official_iou_cache().")
                    for threshold in iou_thresholds:
                        tp, fp, fn = _counts_for_selection(official_iou.float(), selected_ids, threshold)
                ...
                rows.append(row)
                f05 = metrics.get("0.50", {"F1": 0.0})["F1"]
                f07 = metrics.get("0.70", {"F1": 0.0})["F1"]
                print(
                    f"{stage_name:>8} q={quality_power:>4.2f} thr={score_threshold:>4.2f} "
                    f"F1@0.50={f05:.4f} F1@0.70={f07:.4f} mF1={mean_f1:.4f} "
                    f"avg_lanes={row['avg_selected_lanes_per_image']:.2f}"
                )

    rows = sorted(rows, key=lambda row: _row_sort_key(row, args.selection_iou, args.tie_break_iou), reverse=True)
    ...
    report_lines = [
        ...
        f"  iou_thresholds: {' '.join(f'{v:.2f}' for v in iou_thresholds)}",
    ]
    if best is not None:
        report_lines.extend(
            [
    ...
    for row in rows:
        m05 = row["metrics"].get("0.50", {"F1": 0.0})["F1"]
        m07 = row["metrics"].get("0.70", {"F1": 0.0})["F1"]
        report_lines.append(
            f"  q={row['quality_power']:.2f} thr={row['score_threshold']:.2f} "
            f"F1@0.50={m05:.4f} F1@0.70={m07:.4f} mF1={row['mF1']:.4f} "
            f"avg_lanes={row['avg_selected_lanes_per_image']:.2f}"
        )
    report = "\n".join(report_lines) + "\n"
    print(report)
