...
from dynlaneseq_eg.tools.evaluate_v25_g4_denoising_pair import _configured
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Evaluate paired V25 G5 full-population tail emphasis."
    )
...
def _write_and_measure(
    ...
    *,
    ...
    totals = {
        "tail_lanes": 0.0,
        "control_tail_iou": 0.0,
        "treatment_tail_iou": 0.0,
        "control_tail_tp50": 0.0,
        "treatment_tail_tp50": 0.0,
        "control_tail_tp75": 0.0,
        "treatment_tail_tp75": 0.0,
    }
    images_written = 0
    ...
    for batch_index, (images, targets, metas) in enumerate(loader, start=1):
        ...
        with torch.autocast(device_type=device.type, enabled=False):
            control_output = control(images.float())
            treatment_output = treatment(images.float())
        outputs = {
            "control": control_output,
            "tail_emphasis": treatment_output,
        }
        ...
        ordered = build_ordered_lane_targets(
            ...
            device=device,
            slots=4,
            rows=int(control_output["pred_x_rows"].shape[-1]),
            input_w=input_w,
            minimum_valid_rows=minimum_valid_rows,
        )
        valid = ordered["valid_mask"] & ordered["active"].unsqueeze(-1)
        control_iou = _aligned_strip_iou(
            control_output["pred_x_rows"],
            control_output["range_norm"],
            ordered["x_rows"],
            valid,
            line_width=line_width,
        )
        treatment_iou = _aligned_strip_iou(
            treatment_output["pred_x_rows"],
            treatment_output["range_norm"],
            ordered["x_rows"],
            valid,
            line_width=line_width,
        )
        tail = ordered["active"] & (control_iou < 0.60)
        tail_float = tail.float()
        totals["tail_lanes"] += float(tail.sum().item())
        totals["control_tail_iou"] += float(
            (control_iou * tail_float).sum().item()
        )
        totals["treatment_tail_iou"] += float(
            (treatment_iou * tail_float).sum().item()
        )
        for threshold, suffix in ((0.50, "50"), (0.75, "75")):
            totals["control_tail_tp" + suffix] += float(
                ((control_iou >= threshold) & tail).sum().item()
            )
            totals["treatment_tail_tp" + suffix] += float(
                ((treatment_iou >= threshold) & tail).sum().item()
            )
    ...
    count = max(totals["tail_lanes"], 1.0)
    totals["control_tail_mean_iou"] = totals["control_tail_iou"] / count
    totals["treatment_tail_mean_iou"] = totals["treatment_tail_iou"] / count
    totals["images_written"] = float(images_written)
...
def main() -> None:
    ...
    if len(loader.dataset) != expected:
        raise ValueError("G5 evaluator altered official validation population")
    ...
    if control_iteration != treatment_iteration:
        raise ValueError("G5 paired endpoints have different iterations")
    output_dir = Path(args.output_dir).expanduser().resolve()
    directories = {
        policy: output_dir / "predictions" / policy
        for policy in ("control", "tail_emphasis")
    }
    writer = _write_and_measure(
        ...
        line_width=float(control_cfg["v25"]["loss"]["line_width"]),
        minimum_valid_rows=int(
    ...
    if int(writer["images_written"]) != expected:
        raise RuntimeError("G5 writer did not consume full official validation")
    ...
    delta50 = 100.0 * (
        float(metrics["tail_emphasis"]["0.5"]["F1"])
        - float(metrics["control"]["0.5"]["F1"])
    )
    delta75 = 100.0 * (
        float(metrics["tail_emphasis"]["0.75"]["F1"])
        - float(metrics["control"]["0.75"]["F1"])
    )
    checks = {
        "tail_population_nonempty": int(writer["tail_lanes"]) > 0,
        "tail_mean_iou_strictly_better": float(
            writer["treatment_tail_mean_iou"]
        )
        > float(writer["control_tail_mean_iou"]),
        "tail_tp50_not_lower": int(writer["treatment_tail_tp50"])
        >= int(writer["control_tail_tp50"]),
        "full_f1_50_not_worse_by_0p10_points": delta50 >= -0.10,
        "full_f1_75_not_worse_by_0p10_points": delta75 >= -0.10,
    }
    report = {
        "experiment": "V25 G5 capped full-population tail-emphasis paired gate",
        ...
        "gate": {
            ...
            "treatment_minus_control_tail_mean_iou": float(
                writer["treatment_tail_mean_iou"]
            )
            - float(writer["control_tail_mean_iou"]),
        },
        "contract": {
            "official_training_rows_removed": 0,
    ...
    path = output_dir / "g5_official_val_report.json"
    path.write_text(
