...
class LaneTransforms:
    ...
    def __call__(
        ...
        # ``np.random`` preserves the historical contract. Resume-safe
        # training passes a sample-local RandomState so worker scheduling and
        # process restarts cannot change augmentation.
        ...
        if training and self.cfg.affine_prob > 0 and random_source.random() < self.cfg.affine_prob:
            image, seg_mask, lanes = self._apply_affine(image, seg_mask, lanes, random_source)

        if training and self.cfg.horizontal_flip_prob > 0:
            if random_source.random() < self.cfg.horizontal_flip_prob:
        ...
        if training and self.cfg.color_jitter:
            factor = float(random_source.uniform(0.85, 1.15))
            image = ImageEnhance.Brightness(image).enhance(factor)
            factor = float(random_source.uniform(0.85, 1.15))
            image = ImageEnhance.Contrast(image).enhance(factor)
        if training and self.cfg.hue_saturation_prob > 0 and random_source.random() < self.cfg.hue_saturation_prob:
            image = self._jitter_hue_saturation(image, random_source)
        if training and self.cfg.blur_prob > 0 and random_source.random() < self.cfg.blur_prob:
            image = image.filter(ImageFilter.GaussianBlur(radius=float(random_source.uniform(0.4, 1.2))))
        if training and self.cfg.channel_shuffle_prob > 0 and random_source.random() < self.cfg.channel_shuffle_prob:
            arr = np.asarray(image.convert("RGB"), dtype=np.uint8)
            order = random_source.permutation(3)
            image = Image.fromarray(arr[..., order], mode="RGB")
        if training and self.cfg.random_shadow_prob > 0 and random_source.random() < self.cfg.random_shadow_prob:
            image = self._apply_random_shadow(image, random_source)
    ...
    def _apply_affine(
        ...
        w, h = image.size
        angle = float(rng.uniform(-self.cfg.affine_rotate_deg, self.cfg.affine_rotate_deg))
        scale = float(rng.uniform(self.cfg.affine_scale_min, self.cfg.affine_scale_max))
        tx = float(rng.uniform(-self.cfg.affine_translate_x, self.cfg.affine_translate_x) * w)
        ty = float(rng.uniform(-self.cfg.affine_translate_y, self.cfg.affine_translate_y) * h)
    ...
    @staticmethod
    def _jitter_hue_saturation(image: Image.Image, rng: Any) -> Image.Image:
        hsv = np.asarray(image.convert("HSV"), dtype=np.uint8).copy()
        hue_delta = int(rng.randint(-10, 11))
        sat_scale = float(rng.uniform(0.85, 1.15))
    ...
    def _apply_random_shadow(self, image: Image.Image, rng: Any) -> Image.Image:
        ...
        num_vertices = int(rng.randint(min_vertices, max_vertices + 1))
        points = []
        for _ in range(num_vertices):
            x = int(rng.randint(0, max(w, 1)))
            y = int(rng.randint(max(0, min(roi_start, h - 1)), max(h, 1)))
            points.append((x, y))
        ...
        opacity = float(rng.uniform(min_opacity, max_opacity))
