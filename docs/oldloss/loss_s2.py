...
class S2Criterion(S1Criterion):
    ...
    def compute_active_offset_losses(self, outputs, targets, matches):
        ...
        clamp_count = logits.new_tensor(0.0)
        for bi, match in enumerate(matches):
            ...
            reg = F.smooth_l1_loss(
                pred / float(self.cfg.input_w),
                safe_target_delta / float(self.cfg.input_w),
            ...
            total_reg = total_reg + (reg * valid_f).sum()
            total_ce = total_ce + (ce * valid_f).sum()
            count = count + valid_f.sum()
            clamp_count = clamp_count + ((raw_delta.abs() > offset_max) & valid).to(dtype=logits.dtype).sum()
        denom = count.clamp_min(1.0)
        clamp_ratio = clamp_count / denom
        return {
