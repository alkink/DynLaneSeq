...
OUT_DIR="${OUT_DIR:-outputs/diagnostics/dla34_rowref_from55k_evidence1e5_to75k}"
...
if [[ -f "${FINAL_CHECKPOINT}" ]]; then
  echo "The uninterrupted 1e-5 trajectory already reached 75k: ${FINAL_CHECKPOINT}"
  exit 0
fi

# A process restart recreates the DataLoader and augmentation RNG stream.
# Silently resuming a partial 60/65/70k run would therefore destroy the
# iteration-matched comparison with the uninterrupted 2e-6 control.
partial_checkpoint=""
if [[ -d "${OUT_DIR}" ]]; then
  partial_checkpoint="$(
...
if [[ -n "${partial_checkpoint}" ]]; then
  echo "Refusing a causally mismatched partial resume from: ${partial_checkpoint}" >&2
  echo "Use a new empty OUT_DIR and restart from the exact 55k source checkpoint." >&2
  exit 1
fi
...
if iteration != 55000:
    raise SystemExit(f"expected exact iteration 55000, got {iteration} in {path}")
expected = {
    "backbone_decay": 1e-6,
...
echo "Uninterrupted evidence-LR trajectory: ${START_ITERATION} -> ${TARGET_ITERATION}"
echo "Resume with exact 55k AdamW moments: ${SOURCE_CHECKPOINT}"
...
echo "Checkpoints: 60k, 65k, 70k, 75k"
echo "Output: ${OUT_DIR}"
