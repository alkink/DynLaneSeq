...
CKPT="${CKPT:-outputs/culane_s0_structured_query_res34_slots32_b8x2_1600x640_bins800_fpn256_l2_dfl_seed3407_278k/iter_0225000.pt}"
DEVICE="${DEVICE:-cuda}"
...
read -r CKPT_ITER CKPT_LAYERS CKPT_INTRA CKPT_SEED < <(
  python -c 'import sys, torch; p=torch.load(sys.argv[1], map_location="cpu"); c=p.get("cfg", {}); s=c.get("model", {}).get("structured_query", {}); print(int(p.get("iteration", 0)), s.get("num_layers"), s.get("use_intra_attention", True), c.get("seed"))' "${CKPT}"
)
if [[ "${CKPT_LAYERS}" != "2" || "${CKPT_INTRA}" != "True" || "${CKPT_SEED}" != "3407" ]]; then
  echo "Refusing checkpoint: iteration=${CKPT_ITER}, layers=${CKPT_LAYERS}, use_intra=${CKPT_INTRA}, seed=${CKPT_SEED}; expected L2 full seed3407." >&2
  exit 1
fi
