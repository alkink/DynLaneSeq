...
OUTPUT_JSON="${OUTPUT_JSON:-${CHECKPOINT_ROOT}/diagnostics/refinement_arbitration/dla34_225k_uniform64.json}"
...
FUSION_ALPHAS="${FUSION_ALPHAS:-0.01 0.025 0.05 0.1}"
GATE_BUDGETS="${GATE_BUDGETS:-1 2 4}"
ELIGIBLE_TOP_K="${ELIGIBLE_TOP_K:-4}"
QUALITY_POWER="${QUALITY_POWER:-0.5}"

mkdir -p "$(dirname "${OUTPUT_JSON}")"

# shellcheck disable=SC2086
"${PYTHON}" -u -m dynlaneseq_eg.tools.analyze_refinement_arbitration \
  --config "${CONFIG}" \
  --checkpoint "${CKPT}" \
  ...
  --gate-budgets ${GATE_BUDGETS} \
  --eligible-top-k "${ELIGIBLE_TOP_K}" \
