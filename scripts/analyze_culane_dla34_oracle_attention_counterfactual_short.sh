...
OUTPUT_JSON="${OUTPUT_JSON:-${CHECKPOINT_ROOT}/diagnostics/oracle_attention_counterfactual/dla34_225k_uniform64.json}"
...
"${PYTHON}" -u -m \
  dynlaneseq_eg.tools.analyze_oracle_attention_counterfactual \
  --config "${CONFIG}" \
  --checkpoint "${CKPT}" \
