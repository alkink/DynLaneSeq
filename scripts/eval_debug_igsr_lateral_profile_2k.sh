...
CONFIG="dynlaneseq_eg/configs/debug/culane_igsr_gate_lateral_profile_2k.yaml"
OUT_DIR="outputs/debug_igsr_gate_lateral_profile_2k"
...
for threshold in 0.40 0.45 0.50 0.55; do
  ...
  "${PYTHON_BIN}" -m dynlaneseq_eg.tools.evaluate_culane \
    ...
    --pred-dir "${OUT_DIR}/sweep_last_thr${label}_q0p0" \
    --sequential
done
...
"${PYTHON_BIN}" -u -m dynlaneseq_eg.tools.analyze_active_corridor_evidence \
  ...
  --output-json outputs/active_corridor_diagnostics/lateral_profile.json
