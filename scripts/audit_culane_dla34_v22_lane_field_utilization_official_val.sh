...
FIELD_CHECKPOINT="${FIELD_CHECKPOINT:-${OUTPUT_ROOT}/train/lane_field_endpoint.pt}"
...
OUTPUT_JSON="${OUTPUT_JSON:-${OUTPUT_ROOT}/official_val_field_utilization_autopsy.json}"

for required in \
  "${FIELD_CONFIG}" "${V20_CONFIG}" "${V20_GEOMETRY_CHECKPOINT}" \
  "${V20_SCORING_CHECKPOINT}" "${FIELD_CHECKPOINT}" \
  "${VAL_CACHE_MANIFEST}" "${VAL_LIST}"; do
  if [[ ! -f "${required}" ]]; then
    echo "Missing V22 utilization artifact: ${required}" >&2
    exit 1
  fi
done

"${PYTHON}" -u -m dynlaneseq_eg.tools.audit_v22_lane_field_utilization \
...
echo "V22 field-utilization autopsy finished on all 9,675 official val.txt rows. No filtering, deduplication, training, checkpoint/threshold selection, or test evaluation was performed."
