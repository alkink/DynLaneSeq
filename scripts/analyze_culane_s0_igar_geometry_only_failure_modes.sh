...
CKPT="${CKPT:-outputs/culane_s0_igar_geometry_only_25k/iter_0025000.pt}"
SPLIT="${SPLIT:-val}"
EVAL_LIST="${EVAL_LIST:-dataset/list/val.txt}"
DEVICE="${DEVICE:-cuda}"
CACHE_DIR="${CACHE_DIR:-outputs/diagnostic_cache}"
OUT_DIR="${OUT_DIR:-outputs/culane_s0_igar_geometry_only_25k/failure_diagnostics_val}"
...
python -u -m dynlaneseq_eg.tools.analyze_oracle_topk \
  ...
  --output-json "${OUT_DIR}/oracle_topk_${STAGE:-final}.json"
