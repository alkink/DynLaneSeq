...
BASE_OUT_DIR=${BASE_OUT_DIR:-outputs/culane_s0_structured_query_res34_b16_50ep/failure_diagnostics_val_175k}
FEATURE_CACHE=${FEATURE_CACHE:-${BASE_OUT_DIR}/affine_noharm_features.pt}
CV_OUT_DIR=${CV_OUT_DIR:-${BASE_OUT_DIR}/affine_noharm_sequence_cv}
DEVICE=${DEVICE:-cuda}
EPOCHS=${EPOCHS:-100}
...
for SPLIT_SEED in ${SPLIT_SEEDS}; do
  echo "sequence split seed: ${SPLIT_SEED}"
...
python -u -m dynlaneseq_eg.tools.summarize_affine_noharm_cv \
  ...
  --output-json "${CV_OUT_DIR}/summary.json"
