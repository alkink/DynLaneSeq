...
OUTPUT_ROOT="${OUTPUT_ROOT:-/home/alki/projects/DynLaneSeq/outputs/diagnostics/frozen_p2_centerline_separability}"
OUTPUT_JSON="${OUTPUT_JSON:-${OUTPUT_ROOT}/dla34_225k_uniform64_1000steps.json}"
SAVE_PROBES="${SAVE_PROBES:-${OUTPUT_ROOT}/dla34_225k_uniform64_1000steps.pt}"

mkdir -p "${OUTPUT_ROOT}"

"${PYTHON}" -m dynlaneseq_eg.tools.probe_frozen_p2_centerline_separability \
  --config "${CONFIG}" \
  --checkpoint "${CKPT}" \
