...
DEVICE="${DEVICE:-cuda}"
CONFIG="${CONFIG:-dynlaneseq_eg/configs/culane_s0_structured_query_res101_slots32_b8x2_1600x640_bins800_fpn256_l4_dfl_50ep.yaml}"
OUT_DIR="${OUT_DIR:-outputs/culane_s0_structured_query_res101_slots32_b8x2_1600x640_bins800_fpn256_l4_dfl_50ep}"
...
if [[ ! -f "${RESUME}" ]]; then
  ...
  echo "Set RESUME=/path/to/checkpoint.pt or check OUT_DIR." >&2
  exit 1
fi
...
if (( START_ITER <= 0 )); then
  ...
  echo "This usually means the wrong file was passed, or resume failed to read the training checkpoint." >&2
  exit 1
fi
