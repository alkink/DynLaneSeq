...
OUT_DIR="${OUT_DIR:-outputs/debug_s3_unified_active_corridor_qualitycal_structured_query_2k_from_s0_12k}"
DEVICE="${DEVICE:-cuda}"
...
for qp in "${QUALITY_POWERS[@]}"; do
  q_label="${qp//./p}"
  for thresh in "${SCORE_THRESHOLDS[@]}"; do
    thr_label="${thresh//./p}"
    pred_dir="${OUT_DIR}/sweep_${ckpt_label}_thr${thr_label}_q${q_label}"
    echo
    echo "== checkpoint=${CKPT_NAME} score_thresh=${thresh} quality_score_power=${qp} =="
