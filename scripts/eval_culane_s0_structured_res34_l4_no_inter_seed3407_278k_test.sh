...
CKPT="${CKPT:-outputs/culane_s0_structured_query_res34_slots32_b8x2_1600x640_bins800_fpn256_l4_dfl_no_inter_seed3407_278k/iter_0225000.pt}"
DEVICE="${DEVICE:-cuda}"
...
read -r CKPT_ITER CKPT_INTRA CKPT_INTER CKPT_SEED < <(
  python -c 'import sys, torch; p=torch.load(sys.argv[1], map_location="cpu"); c=p.get("cfg", {}); s=c.get("model", {}).get("structured_query", {}); print(int(p.get("iteration", 0)), s.get("use_intra_attention", True), s.get("use_inter_attention", True), c.get("seed"))' "${CKPT}"
)
if [[ "${CKPT_INTRA}" != "True" || "${CKPT_INTER}" != "False" || "${CKPT_SEED}" != "3407" ]]; then
  echo "Refusing checkpoint: iteration=${CKPT_ITER}, intra=${CKPT_INTRA}, inter=${CKPT_INTER}, seed=${CKPT_SEED}; expected no-inter seed3407." >&2
  exit 1
fi
