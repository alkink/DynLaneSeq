...
RESULT_DIR="${RESULT_DIR:-${CKPT_DIR}/test_eval_${CKPT_TAG}_thr${SCORE_TAG}_q${QUALITY_TAG}_nms${NMS_TAG}}"
PREDICTION_FILE="${PREDICTION_FILE:-${RESULT_DIR}/tusimple_predictions.jsonl}"
...
python -u -m dynlaneseq_eg.tools.evaluate_tusimple \
  --config "${CONFIG}" \
  --checkpoint "${CKPT}" \
  --split test \
