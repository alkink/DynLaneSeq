...
CKPT="${CKPT:-outputs/culane_s0_structured_query_res34_slots32_b8x2_1600x640_bins800_fpn256_l4_dfl_no_intra_seed3407_278k/iter_0225000.pt}"
DEVICE="${DEVICE:-cuda}"
...
if [[ "${CKPT_INTRA}" != "False" || "${CKPT_SEED}" != "3407" ]]; then
  echo "Refusing checkpoint: iteration=${CKPT_ITER}, use_intra=${CKPT_INTRA}, seed=${CKPT_SEED}; expected no-intra seed3407." >&2
  exit 1
fi
...
PRED_DIR="${PRED_DIR:-${CKPT_DIR}/val_eval_${CKPT_TAG}_thr${SCORE_TAG}_q${QUALITY_TAG}_nms${NMS_TAG}}"
