...
OUTPUT_ROOT="${OUTPUT_ROOT:-outputs/diagnostics/unified_lane_set_v5_four_slot_confirmation}"
...
for artifact in "${CONFIG}" "${CHECKPOINT}" "${TRAIN_CACHE}" "${VAL_CACHE}"; do
  if [[ ! -f "${artifact}" ]]; then
    echo "Missing required confirmation artifact: ${artifact}" >&2
    echo "This script is cache-only and will not rerun detector inference." >&2
    exit 1
  fi
done
...
for seed in ${SEEDS}; do
  ...
  mkdir -p "${seed_root}"
  report="${seed_root}/four_slot_parameter_matched_uniform256.json"
  reports+=("${report}")
  echo "===== four-slot confirmation seed ${seed} ====="
  "${PYTHON}" -u -m dynlaneseq_eg.tools.probe_v5_four_slot_router \
    ...
    --seed "${seed}" \
    --output-json "${report}" \
    --save-probes "${seed_root}/four_slot_parameter_matched.pt"
done

"${PYTHON}" -u -m dynlaneseq_eg.tools.summarize_v5_four_slot_confirmation \
  --reports "${reports[@]}" \
  --min-mean-gain-050 5.0 \
  --min-mean-gain-075 2.0 \
  --min-positive-seeds 2 \
  --output-json "${OUTPUT_ROOT}/summary.json"

echo "Four-slot confirmation ready: ${OUTPUT_ROOT}/summary.json"
